  ...
  return (
    <main className="min-h-svh bg-[radial-gradient(circle_at_50%_16%,#fff0df_0,transparent_30%),#fbf7f3] text-[#342721]">
      ...
      >
        ...
        {error && (
          <Alert className="mx-auto mt-7 max-w-2xl border-[#efd7c8] bg-[#fff5ed] text-left text-[#8b523a]">
            ...
            <AlertDescription>
              ...
              <span className="text-xs text-[#94776a]">
                목록 조회 API가 준비되지 않았더라도 새 아이는 등록할 수 있어요.
              </span>
            </AlertDescription>
          </Alert>
        )}
        ...
        {isLoading ? (
          <div
            className="grid min-h-72 place-items-center text-sm text-[#79685f]"
            aria-live="polite"
          >
            아이 프로필을 불러오는 중...
          </div>
          ...
          <>
            ...
            >
              {children.map((child, index) => {
                ...
                const tones = [
                  "bg-[#f2cdb8] text-[#78472f]",
                  "bg-[#dce9d3] text-[#4e6843]",
                  "bg-[#f3dfaa] text-[#765e2e]",
                  "bg-[#d8e6e9] text-[#49636c]",
                ];
                return (
                  <button
                  ...
                  >
                    <Card
                      className={`relative aspect-square overflow-hidden border-2 p-0 transition duration-200 group-hover:-translate-y-1 group-hover:shadow-xl ${selected ? "border-[#75432f] shadow-xl shadow-[#75432f]/15" : "border-transparent shadow-md shadow-[#75432f]/5"}`}
                    >
                      {selected && (
                        <span className="absolute top-2 right-2 z-10 grid size-7 place-items-center rounded-full bg-[#75432f] text-white">
                          <Check className="size-4" />
                        </span>
                      )}
                      <Avatar className="size-full rounded-none" size="lg">
                        <AvatarImage
                        ...
                        />
                        <AvatarFallback
                          className={`rounded-none text-5xl font-black ${tones[index % tones.length]}`}
                        >
                    ...
                    <small className="mt-1 block text-xs text-[#79685f]">
                      만 {getAge(child.birthDate)}세 ·{" "}
              ...
              >
                <Card className="grid aspect-square place-items-center border-2 border-dashed border-[#caa993] bg-white/70 p-0 transition group-hover:-translate-y-1 group-hover:border-[#a8623e] group-hover:shadow-lg">
                  <span className="grid size-14 place-items-center rounded-full bg-[#f8eae1] text-[#a8623e]">
                    <Plus className="size-7" />
                  </span>
                </Card>
                <strong className="mt-3 block text-base">아이 등록</strong>
                <small className="mt-1 block text-xs text-[#79685f]">
                  새 프로필 추가
                </small>
              </button>
            ...
            {!children.length && !error && (
              <p className="mt-6 text-sm text-[#79685f]">
      ...
      >
        <DialogContent className="max-h-[calc(100svh-2rem)] overflow-y-auto border-[#eadfd8] bg-[#fffdfb] sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogTitle className="text-3xl tracking-[-.05em] text-[#342721]">
              아이 등록
            </DialogTitle>
            <DialogDescription className="text-[#79685f]">
          ...
          <form className="space-y-4" onSubmit={handleRegister}>
            ...
            <div className="space-y-2">
              <Label htmlFor="child-image">
                프로필 사진{" "}
                <span className="font-normal text-[#79685f]">(선택)</span>
              </Label>
              <Input
                id="child-image"
                name="profileImage"
            ...
            <Alert className="border-[#ead8cc] bg-[#fff7f1] text-[#866b5e]">
              <Info />
              <AlertDescription>
                JPEG, PNG, WebP 파일을 최대 5MB까지 등록할 수 있어요. 아이 생성
                후 사진이 이어서 업로드됩니다.
              </AlertDescription>
            </Alert>
            {registerError && (
            ...
            <DialogFooter className="-mx-4 -mb-4 mt-5">
              ...
              <Button
                type="submit"
                disabled={isMutating}
                className="bg-[#75432f] text-white hover:bg-[#5f3525]"
              >
